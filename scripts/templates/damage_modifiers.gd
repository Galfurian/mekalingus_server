# This script defines how different damage types interact with armor, shields,
...
static func apply_effect_damage(effect: ItemEffect, target: Mek) -> Dictionary:
	...
	match effect.damage_type:
		Enums.DamageType.KINETIC:
			reduction += target.damage_reduction_kinetic
		Enums.DamageType.ENERGY:
			reduction += target.damage_reduction_energy
		Enums.DamageType.EXPLOSIVE:
			reduction += target.damage_reduction_explosive
		Enums.DamageType.PLASMA:
			reduction += target.damage_reduction_plasma
		Enums.DamageType.CORROSIVE:
			reduction += target.damage_reduction_corrosive
